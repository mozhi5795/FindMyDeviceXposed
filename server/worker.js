...
function getTileUrl(){var u=localStorage.getItem('fmd_tile_url');return u||'https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png';}function getTileOpts(u){var o={attribution:'Map',maxZoom:19};if(u.indexOf('{s}')>=0){if(u.indexOf('webrd')>=0||u.indexOf('wprd')>=0||u.indexOf('autonavi')>=0){if(u.indexOf('webrd01')>=0||u.indexOf('wprd01')>=0){o.subdomains=['01','02','03','04']}else{o.subdomains=['1','2','3','4']}}}return o}function initMap(){MAP=L.map('map',{center:[35,105],zoom:5,zoomControl:true});var u=getTileUrl();L.tileLayer(u,getTileOpts(u)).addTo(MAP);}
...
c.innerHTML=dev.map(function(d){var on=d.online&&isRecent(d.lastSeen),ago=timeAgo(d.lastSeen),sel=d.token===SELECTED_TOKEN?'selected':'';return'<div class="device-card '+sel+'" onclick="selectDevice(\\''+d.token+'\\')"><div class="device-name"><span class="'+(on?'online':'offline')+'">●</span> '+(d.model||d.token.substring(0,16))+'</div><div class="device-meta">'+(on?'在线':'离线')+' · '+ago+(d.lastBattery>0?' · 电量 '+d.lastBattery+'%':'')+(d.locationCount>0?' · '+d.locationCount+' 点':'')+'</div></div>';}).join('');}
function selectDevice(token){SELECTED_TOKEN=token;loadDevices();document.getElementById('device-detail').style.display='block';loadDetail(token);var m=DEVICE_MARKERS[token];if(m){MAP.setView(m.getLatLng(),15);m.openPopup();}loadHistory(token);}
function loadDetail(token){fetch('/api/device/'+encodeURIComponent(token),{credentials:'same-origin'}).then(function(r){return r.json();}).then(function(info){document.getElementById('detail-token').textContent='📱 '+(info.model||'未知');var on=info.online&&isRecent(info.lastSeen);document.getElementById('detail-info').innerHTML='<div>状态: '+(on?'<span style="color:#4CAF50">在线</span>':'<span style="color:#f44336">离线</span>')+'</div><div>标识: <code style="font-size:11px;color:#2196F3">'+token+'</code></div><div>型号: '+(info.model||'未知')+'</div><div>最后位置: '+(info.lastLat?info.lastLat.toFixed(6)+', '+info.lastLng.toFixed(6):'未知')+'</div><div>精度: '+(info.lastAccuracy?info.lastAccuracy+'米':'未知')+'</div><div>定位源: '+(info.lastProvider||'未知')+'</div><div>电量: '+(info.lastBattery>0?info.lastBattery+'%':'未知')+'</div><div>位置点数: '+((info.locations||[]).length)+'</div><div>首次: '+fmtTime(info.firstSeen)+'</div><div>最后活跃: '+fmtTime(info.lastSeen)+'</div>';updateBar({token:token,lat:info.lastLat,lng:info.lastLng,accuracy:info.lastAccuracy,battery:info.lastBattery,model:info.model,time:info.lastSeen,online:on});}).catch(function(){});}
...
function updateMarker(d){if(!d.lat||!d.lng)return;var ll=[d.lat,d.lng];if(DEVICE_MARKERS[d.token]){DEVICE_MARKERS[d.token].setLatLng(ll);}else{var ic=L.divIcon({html:'<div style="background:#e94560;color:#fff;border-radius:50%;width:32px;height:32px;display:flex;align-items:center;justify-content:center;font-size:16px;border:3px solid #fff;box-shadow:0 2px 8px rgba(0,0,0,.5)">📱</div>',className:'',iconSize:[32,32],iconAnchor:[16,16]});var m=L.marker(ll,{icon:ic}).addTo(MAP);m.bindPopup('<b>'+(d.model||d.token.substring(0,10))+'</b><br>精度: '+(d.accuracy?d.accuracy+'米':'未知')+'<br>电量: '+(d.battery>0?d.battery+'%':'未知'));DEVICE_MARKERS[d.token]=m;}if(d.token===SELECTED_TOKEN&&MAP.getZoom()<14)MAP.setView(ll,15);}
function loadHistory(token){if(token){fetch('/api/device/'+encodeURIComponent(token),{credentials:'same-origin'}).then(function(r){return r.json();}).then(function(info){var locs=info.locations||[];if(locs.length<2)return;if(window._trailLine)MAP.removeLayer(window._trailLine);if(window._trailStart)MAP.removeLayer(window._trailStart);if(window._trailEnd)MAP.removeLayer(window._trailEnd);var pts=locs.map(function(l){return[l.lat,l.lng];});window._trailLine=L.polyline(pts,{color:'#e94560',weight:3,opacity:.6}).addTo(MAP);window._trailStart=L.circleMarker(pts[0],{radius:5,color:'#4CAF50',fillColor:'#4CAF50',fillOpacity:1}).addTo(MAP).bindPopup('最早');window._trailEnd=L.circleMarker(pts[pts.length-1],{radius:5,color:'#f44336',fillColor:'#f44336',fillOpacity:1}).addTo(MAP).bindPopup('最新');MAP.fitBounds(pts);}).catch(function(){});}else{fetch('/api/commands/history',{credentials:'same-origin'}).then(function(r){return r.json();}).then(function(items){var c=document.getElementById('history-list');var hint=c.querySelector('.hint');if(hint)hint.remove();if(!items||!items.length){c.innerHTML='<p class="hint">暂无记录</p>';return;}c.innerHTML=items.slice(-30).reverse().map(function(h){return'<div class="history-item"><span class="h-time">'+fmtTime(h.time)+'</span><span class="h-device">'+((h.token||'').substring(0,10))+'</span><span class="h-action">'+(h.action||'--')+'</span><span class="h-result'+(h.result&&h.result.startsWith('error')?' error':'')+'">'+(h.result||'--')+'</span></div>';}).join('');}).catch(function(){});}}
